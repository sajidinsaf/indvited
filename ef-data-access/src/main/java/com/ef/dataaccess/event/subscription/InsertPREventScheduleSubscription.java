...
public class InsertPREventScheduleSubscription
  ...
  private final String INSERT_SCHEDULE_TIMESLOT_SUBSCRIPTION_STATEMENT = "INSERT INTO event_schedule_timeslot_subscription (event_schedule_timeslot_id, subscriber_id, priority, status_id) VALUES (?,?,?,?)";
  ...
  public EventScheduleSubscription data(final PREventScheduleSubscriptionBindingModel input) {
    ...
    EventScheduleSubscription eventScheduleSubscription = new EventScheduleSubscription(subscriptionId,
        input.getEventSubscriptionTimeslotId(), input.getSubscriberId(), input.getPriority(),
  ...
  private final PreparedStatement getInsertEventScheduleSubscriptionBindingModel(
    ...
    PreparedStatement ps = connection.prepareStatement(INSERT_SCHEDULE_TIMESLOT_SUBSCRIPTION_STATEMENT,
        Statement.RETURN_GENERATED_KEYS);
    ps.setLong(1, input.getEventSubscriptionTimeslotId());
    ps.setInt(2, input.getSubscriberId());
    ps.setInt(3, input.getPriority());
